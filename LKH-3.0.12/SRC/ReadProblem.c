...
static void Read_GCTSP_SECTION()
{
    ...
    for (j = 1; j <= Salesmen; j++) {
        ...
        for (i = 1; i <= Dimension; i++) {
            ...
            if (b != 0 && b != 1)
                eprintf("GCTSP_SECTION: %d, 0 or 1 expected", b);
            ColorAllowed[j][i] = b;
        }
    }
    for (i = 1; i <= Dimension; i++) {
        Colors = 0;
        for (j = 1; j <= Salesmen; j++)
            Colors += ColorAllowed[j][i];
        if (Colors == 0)
            eprintf("GCTSP_SECTION: Node %d has no colors", i);
    }
}
...
static void Read_GCTSP_SET_SECTION()
{
    ...
    while (fscanf(ProblemFile, "%d", &Id) > 0) {
        ...
        if (Colors == 0)
            eprintf("GCTSP_SET_SECTION: Node %d has no colors", Id);
    }
